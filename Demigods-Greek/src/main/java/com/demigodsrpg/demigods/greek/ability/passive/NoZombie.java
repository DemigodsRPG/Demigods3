    ...
    public NoZombie(final String deity) {
        super(NAME, deity, REPEAT, DETAILS, new Listener() {
            ...
            public void onEntityTargetEntity(EntityTargetLivingEntityEvent targetEvent) {
                if (Zones.inNoDemigodsZone(targetEvent.getEntity().getLocation()) || !(targetEvent.getTarget() instanceof Player))
                    return;
                if (targetEvent.getEntity() instanceof Zombie && Deity.Util.canUseDeitySilent(DemigodsCharacter.of((Player) targetEvent.getTarget()), deity))
            ...
            public void run() {
                for (DemigodsCharacter character : Demigods.getServer().getOnlineCharactersWithDeity(deity)) {
                    ...
                    for (Entity entity : player.getNearbyEntities(15, 15, 15))
                        if (entity instanceof Zombie && ((Zombie) entity).getTarget() != null && ((Zombie) entity).getTarget().equals(player))
