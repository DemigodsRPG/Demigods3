...
import com.demigodsrpg.demigods.engine.util.Vehicles;
...
public class MoveListener implements Listener {
    ...
    private static void onBattleMoveEvent(Entity entity, Location to, Location from) {
        if (!Configs.getSettingBoolean("battles.enabled") || !Battle.canParticipate(entity) || entity.isInsideVehicle())
            return;
        ...
        if (Battle.isInBattle(participant)) {
            ...
            if (toBool && !fromBool)
                TempDataManager.saveTemp((participant.getEntity().getPassenger() == null ? participant.getId().toString() : participant.getRelatedCharacter().getId().toString()), "battle_safe_location", from);
            if (toBool) {
