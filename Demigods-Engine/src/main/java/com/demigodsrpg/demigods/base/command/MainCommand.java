		...
		public boolean process(Player sender, String[] args)
		{
			...
			if("player".equalsIgnoreCase(args[2]))
			{
				...
				if(player != null)
				{
					...
					if(player.isACharacter()) sender.sendMessage(ChatColor.GRAY + " " + Symbol.RIGHTWARD_ARROW + " " + ChatColor.RESET + "Current Character: " + player.getCharacter().getDeity().getColor() + player.getCharacter().getName());
