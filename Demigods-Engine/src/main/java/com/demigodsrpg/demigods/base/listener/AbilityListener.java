...
public class AbilityListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent interactEvent) {
        if (Zones.inNoDemigodsZone(interactEvent.getPlayer().getLocation()) || !Ability.Util.isLeftClick(interactEvent))
            return;

        // Set variables
        final Player player = interactEvent.getPlayer();
        final DemigodsCharacter character = DemigodsCharacter.of(player);
