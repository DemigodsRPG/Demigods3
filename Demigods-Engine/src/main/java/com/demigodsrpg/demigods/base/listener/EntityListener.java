...
public class EntityListener implements Listener
{
	...
	public static void damageEvent(EntityDamageEvent event)
	{
		...
		if(event.getEntity() instanceof Player && !Battle.canTarget(Battle.defineParticipant(event.getEntity()))) event.setCancelled(true);
	}
	...
	{
		if(Zones.inNoDemigodsZone(event.getEntity().getLocation())) return;
		if(event.getTarget() instanceof Player && !DemigodsPlayer.of(((Player) event.getTarget())).canPvp()) event.setCancelled(true);
	}
}
