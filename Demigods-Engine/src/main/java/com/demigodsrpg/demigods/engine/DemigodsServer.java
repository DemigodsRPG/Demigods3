...
import com.demigodsrpg.demigods.engine.helper.CensoredCentralizedClass;
...
public class DemigodsServer extends CensoredCentralizedClass {
    ...
    protected void loadListeners() {
        ...
        for (DemigodsStructureType structureType : Collections2.filter(getMythos().getStructures(), new Predicate<DemigodsStructureType>() {
            ...
            if (structureType.getUniqueListener() != null)
                register.registerEvents(structureType.getUniqueListener(), DemigodsPlugin.getInst());
        ...
        for (ConversationManager conversation : Collections2.filter(Collections2.transform(Sets.newHashSet(DemigodsConversation.values()), new Function<DemigodsConversation, ConversationManager>() {
            ...
            if (conversation.getUniqueListener() != null)
                register.registerEvents(conversation.getUniqueListener(), DemigodsPlugin.getInst());

        // Divine Items
        for (DivineItem divineItem : getMythos().getDivineItems()) {
            if (divineItem.getUniqueListener() != null)
                register.registerEvents(divineItem.getUniqueListener(), DemigodsPlugin.getInst());
