...
public class WorldDataManager implements Listener {
    ...
    protected <V extends WorldDataAccess<K, V>, K> V getFor(Class clazz, DemigodsWorld world, K key) {
        if (getMapFor(clazz, world).containsKey(key)) return (V) (WorldDataAccess) getMapFor(clazz, world).get(key);
        return null;
    }

    protected <K, V extends WorldDataAccess<K, V>> Collection<V> getAllOf(Class clazz, DemigodsWorld world) {
        return ((ConcurrentMap<K, V>) getMapFor(clazz, world)).values();
    }

    protected <K, V extends WorldDataAccess<K, V>> ConcurrentMap<K, V> getMapFor(Class clazz, DemigodsWorld world) {
    ...
    protected <K, V extends WorldDataAccess<K, V>> void putFor(Class clazz, DemigodsWorld world, K key, V value) {
        ((ConcurrentMap<K, V>) getMapFor(clazz, world)).put(key, value);
    }

    protected <K, V extends WorldDataAccess<K, V>> void removeFor(Class clazz, DemigodsWorld world, K key) {
        ((ConcurrentMap<K, V>) getMapFor(clazz, world)).remove(key);
    }
