...
public class DemigodsCharacter extends DataAccess<UUID, DemigodsCharacter> implements Participant {
    ...
    public DemigodsEnderInventory getDemigodsEnderInventory() {
        if (DemigodsEnderInventory.get(enderInventory) == null)
            enderInventory = DemigodsEnderInventory.create(this).getId();
    ...
    public void remove() {
        ...
        if (getBukkitOfflinePlayer().isOnline() && demigodsPlayer.getCharacter().getName().equalsIgnoreCase(getName()))
            demigodsPlayer.setToMortal();

        // Remove the data
        if (demigodsPlayer.getCharacter() != null && demigodsPlayer.getCharacter().getName().equalsIgnoreCase(getName()))
            demigodsPlayer.resetCurrent();
