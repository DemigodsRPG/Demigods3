package com.demigodsrpg.demigods.engine.schematic;
...
public class Selection {
    ...
    public static List<Selection> getCuboid(Location referenceA, Location referenceB, int radius) {
        ...
        for (int x = (X < XX ? X : XX); x <= (X < XX ? XX : X); x++)
            for (int y = (Y < YY ? Y : YY); y <= (Y < YY ? YY : Y); y++)
                for (int z = (Z < ZZ ? Z : ZZ); z <= (Z < ZZ ? ZZ : Z); z++) {
                    Block b = referenceA.getWorld().getBlockAt(x, y, z);
                    selections.add(new Selection(x + differenceX, y + differenceY, z + differenceZ, b.getType(), b.getData()));
                }
        return selections;
    }
}
