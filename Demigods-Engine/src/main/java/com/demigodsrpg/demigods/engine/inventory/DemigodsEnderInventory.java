    ...
    public void setItems(org.bukkit.inventory.Inventory inventory) {
        ...
        for (int i = 0; i < 26; i++) {
            if (inventory.getItem(i) == null)
                this.items[i] = DemigodsItemStack.create(new ItemStack(Material.AIR)).getId().toString();
        ...
        if (this.items != null) {
            ...
            for (int i = 0; i < 26; i++) {
                if (this.items[i] != null) {
                    ...
                    if (itemStack != null)
                        inventory.setItem(i, DemigodsItemStack.get(UUID.fromString(this.items[i])).getBukkitItem());
                }
            }
        }
