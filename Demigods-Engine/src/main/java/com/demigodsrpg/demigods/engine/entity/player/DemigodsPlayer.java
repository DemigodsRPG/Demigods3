...
public class DemigodsPlayer extends DataAccess<UUID, DemigodsPlayer> {
    ...
    public DemigodsPlayer(UUID mojangAccount, ConfigurationSection conf) {
        ...
        if (conf.getString("mortalEnderInventory") != null)
            mortalEnderInventory = UUID.fromString(conf.getString("mortalEnderInventory"));
    }
    ...
    public DemigodsRegion getRegion() {
        if (getBukkitOfflinePlayer().isOnline())
            return DemigodsRegion.at(getBukkitOfflinePlayer().getPlayer().getLocation());
    ...
    public static DemigodsPlayer create(final OfflinePlayer player) {
        ...
        // Log the creation
        Messages.info(English.LOG_PLAYER_CREATED.getLine().replace("{player}", player.getName()).replace("{id}", player.getUniqueId().toString()));
