...
public class Zones {
    ...
    public static int init() {
        ...
        WorldGuards.setWhenToOverridePVP(DemigodsPlugin.getInst(), new Predicate<Event>() {
            @Override
            public boolean apply(Event event) {
                return !(event instanceof EntityDamageByEntityEvent) ||
                        !Zones.inNoDemigodsZone(((EntityDamageByEntityEvent) event).getEntity().getLocation());
            }
        });
    ...
    public static boolean inNoPvpZone(Location location) {
        ...
        if (WorldGuards.worldGuardEnabled())
            return DemigodsStructureType.Util.isInRadiusWithFlag(location, DemigodsStructureType.Flag.NO_PVP) || !WorldGuards.canPVP(location);
