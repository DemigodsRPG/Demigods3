    ...
    static {
        ...
        SAVE = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (SAVE_ALERT)
                    Messages.info(Bukkit.getOnlinePlayers().size() + " of " + Demigods.getServer().getAllPlayers().size() + " total players saved in " + Times.getSeconds(time) + " seconds.");
            }
        };
        ...
        for (Deity deity : Demigods.getMythos().getDeities())
            for (Ability ability : deity.getAbilities())
                if (ability.getRunnable() != null)
                    Bukkit.getScheduler().scheduleSyncRepeatingTask(DemigodsPlugin.getInst(), ability.getRunnable(), ability.getDelay(), ability.getRepeat());
