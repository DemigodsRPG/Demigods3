...
public class Administration implements ConversationManager {
    ...
    static class Generation {
        ...
        static class Menu extends ValidatingPrompt implements DemigodsConversation.Category {
            ...
            private static Map<Integer, DemigodsStructureType> getStructureChoices(ConversationContext context) {
                // Grab the data (that may or may not be there)
                Object data = context.getSessionData("generable_structures");

                // Return it if it exists
                if (data != null) {
                    return (Map<Integer, DemigodsStructureType>) data;
                } else {
                    int count = 1;
                    Map<Integer, DemigodsStructureType> structures = Maps.newHashMap();
                    ...
                    for (DemigodsStructureType structureType : Demigods.getMythos().getStructures()) {
                        ...
                        if (structureType.getFlags().contains(DemigodsStructureType.Flag.STRUCTURE_WAND_GENERABLE)) {
            ...
            public String getPromptText(ConversationContext context) {
                ...
                for (Map.Entry<Integer, DemigodsStructureType> structure : getStructureChoices(context).entrySet()) {
        ...
        static class Design extends ValidatingPrompt {
            private static DemigodsStructureType getChosenStructure(ConversationContext context) {
                return (DemigodsStructureType) context.getSessionData("chosen_structure");
            }

            private static Map<Integer, DemigodsStructureType.Design> getDesignChoices(ConversationContext context) {
                int count = 1;
                Map<Integer, DemigodsStructureType.Design> designs = Maps.newHashMap();

                for (DemigodsStructureType.Design design : getChosenStructure(context).getDesigns()) {
            ...
            public String getPromptText(ConversationContext context) {
                ...
                // Get the structure
                DemigodsStructureType structureType = getChosenStructure(context);
                ...
                for (Map.Entry<Integer, DemigodsStructureType.Design> design : getDesignChoices(context).entrySet()) {
        ...
        static class Selection extends ValidatingPrompt {
            ...
            protected Prompt acceptValidatedInput(ConversationContext context, String message) {
                ...
                if ("menu".equalsIgnoreCase(message)) {
                    ...
                    String design = ((DemigodsStructureType.Design) context.getSessionData("chosen_design")).getName();
            ...
            private static void scheduleGeneration(final Player player, final DemigodsStructureType structureType, @Nullable final String design, final Location... locations) {
    ...
    public static class Listener implements org.bukkit.event.Listener {
        ...
        private void onStructureWand(PlayerInteractEvent event) {
            ...
            if (event.getClickedBlock() == null || Zones.inNoDemigodsZone(event.getPlayer().getLocation()) || !Util.useStructureWand(event.getPlayer()))
                return;
    ...
    public static class Util {
        ...
        public static void clearAdministrationSession(OfflinePlayer player) {
            TempDataManager.removeTemp(player.getName(), "administration_conversation");
            TempDataManager.removeTemp(player.getName(), "administration_context");
        }
        ...
        public static void saveAdministrationContext(Player player, ConversationContext context) {
            TempDataManager.saveTemp(player.getName(), "administration_context", context);
        }
        ...
        public static ConversationContext getAdministrationContext(Player player) {
            ...
            return (ConversationContext) TempDataManager.getValueTemp(player.getName(), "administration_context");
        }
        ...
        public static void toggleAdministration(Player player, boolean option, boolean recordChat) {
            if (option) {
                ...
                TempDataManager.saveTemp(player.getName(), "administration_conversation", conversation);
                ...
                if (TempDataManager.hasKeyTemp(player.getName(), "administration_conversation")) {
                    Conversation conversation = (Conversation) TempDataManager.getValueTemp(player.getName(), "administration_conversation");
                    TempDataManager.saveTemp(player.getName(), "administration_context", conversation.getContext());
        ...
        public static boolean wandEnabled(OfflinePlayer player) {
            return player.getPlayer().hasPermission("demigods.admin") && TempDataManager.hasKeyTemp(player.getName(), "temp_admin_wand") && Boolean.parseBoolean(TempDataManager.getValueTemp(player.getName(), "temp_admin_wand").toString());
        }
        ...
        public static void toggleWand(OfflinePlayer player, boolean option) {
            if (option) {
                TempDataManager.saveTemp(player.getName(), "temp_admin_wand", true);
            } else {
                TempDataManager.removeTemp(player.getName(), "temp_admin_wand");
            }
        }
        ...
        public static boolean structureWandEnabled(OfflinePlayer player) {
            return player.getPlayer().hasPermission("demigods.admin") && TempDataManager.hasKeyTemp(player.getName(), "temp_admin_structurewand") && Boolean.parseBoolean(TempDataManager.getValueTemp(player.getName(), "temp_admin_structurewand").toString());
        }
        ...
        public static void toggleStructureWand(OfflinePlayer player, boolean option) {
            if (option) {
                TempDataManager.saveTemp(player.getName(), "temp_admin_structurewand", true);
            } else {
                TempDataManager.removeTemp(player.getName(), "temp_admin_structurewand");
            }
        }
        ...
        public static void togglePlayerDebug(OfflinePlayer player, boolean option) {
            if (option) {
                TempDataManager.saveTemp(player.getName(), "temp_admin_debug", true);
            } else {
                TempDataManager.removeTemp(player.getName(), "temp_admin_debug");
            }
        }
        ...
        public static boolean playerDebugEnabled(OfflinePlayer player) {
            return player.getPlayer().hasPermission("demigods.admin") && TempDataManager.hasKeyTemp(player.getName(), "temp_admin_debug") && Boolean.parseBoolean(TempDataManager.getValueTemp(player.getName(), "temp_admin_debug").toString());
        }
