...
public class Prayer implements ConversationManager
{
	...
	public static Map<Object, Object> grabRawContext(Player player)
	{
		...
		try
		{
			if(!DemigodsServer.isRunningSpigot())
			{
				...
				if(TempDataManager.hasKeyTemp(player.getName(), CONTEXT_NAME)) context = (Map<Object, Object>) sessionDataField.get(TempDataManager.getValueTemp(player.getName(), CONTEXT_NAME));
			}
			else
			{
				// Grab the context Map
				if(TempDataManager.hasKeyTemp(player.getName(), CONTEXT_NAME)) context.putAll(((ConversationContext) TempDataManager.getValueTemp(player.getName(), CONTEXT_NAME)).getAllSessionData());
			}
		}
	...
	static class ViewWarps extends ValidatingPrompt implements DemigodsConversation.Category
	{
		...
		protected boolean isInputValid(ConversationContext context, String message)
		{
			...
			{
				if((DemigodsCharacter.charExists(arg1) || (DemigodsPlayer.getFromName(arg1) != null && DemigodsPlayer.getFromName(arg1).isACharacter()) && arg2 != null && character.getMeta().getWarps().containsKey(arg2.toLowerCase()))) return true;
		...
		protected Prompt acceptValidatedInput(ConversationContext context, String message)
		{
			...
			else if("warp".equalsIgnoreCase(arg0))
			{
				...
				if(character.getMeta().getWarps().containsKey(arg1.toLowerCase())) player.teleport(DemigodsLocation.get(UUID.fromString(character.getMeta().getWarps().get(arg1.toLowerCase()).toString())).getBukkitLocation());
	...
	static class ViewCharacters extends ValidatingPrompt implements DemigodsConversation.Category
	{
		...
		class DetailedInfo extends ValidatingPrompt
		{
			...
			protected Prompt acceptValidatedInput(ConversationContext context, String message)
			{
				...
				if("switch".equalsIgnoreCase(message)) DemigodsPlayer.of((Player) context.getForWhom()).switchCharacter(Demigods.getServer().getCharacter(context.getSessionData("viewing_character").toString()));
				return null;
			}
		}
	}
		...
		class ChooseName extends ValidatingPrompt
		{
			...
			protected boolean isInputValid(ConversationContext context, String name)
			{
				if(name.length() < 2 || name.length() > 13 || !StringUtils.isAlphanumeric(name) || Strings.hasCapitalLetters(name, Configs.getSettingInt("character.max_caps_in_name")) || DemigodsCharacter.charExists(name) || Strings.containsAnyInCollection(name, getBlackList()))
				{
					...
					if(DemigodsCharacter.charExists(name) || Strings.containsAnyInCollection(name, getBlackList())) errors.add(English.ERROR_CHAR_EXISTS);
	...
	public static class Listener implements org.bukkit.event.Listener
	{
		...
		private void onPlayerMove(PlayerMoveEvent event)
		{
			...
			if(Util.isPraying(player) && event.getTo().distance((Location) TempDataManager.getValueTemp(player.getName(), "prayer_location")) >= Configs.getSettingInt("zones.prayer_radius")) Util.togglePraying(player, false);
		}
	}
