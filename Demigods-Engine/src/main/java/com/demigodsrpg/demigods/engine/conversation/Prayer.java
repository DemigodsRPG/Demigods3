...
public class Prayer implements ConversationManager {
    // Define constants
    ...
    public static Map<Object, Object> grabRawContext(Player player) {
        ...
        try {
            if (!DemigodsServer.isRunningSpigot()) {
                ...
                if (TempDataManager.hasKeyTemp(player.getName(), CONTEXT_NAME))
                    context = (Map<Object, Object>) sessionDataField.get(TempDataManager.getValueTemp(player.getName(), CONTEXT_NAME));
            } else {
                // Grab the context Map
                if (TempDataManager.hasKeyTemp(player.getName(), CONTEXT_NAME))
                    context.putAll(((ConversationContext) TempDataManager.getValueTemp(player.getName(), CONTEXT_NAME)).getAllSessionData());
            }
    ...
    public static Conversation startPrayer(Player player) {
        ...
        Conversation conversation = Demigods.getConversationFactory().withEscapeSequence("/exit").withLocalEcho(false).withInitialSessionData(grabRawContext(player)).withFirstPrompt(new StartPrayer()).buildConversation(player);
        conversation.begin();
    ...
    static class StartPrayer extends ValidatingPrompt {
        ...
        public String getPromptText(ConversationContext context) {
            ...
            // Send menu options
            for (Menu menu : Menu.values())
                if (menu.getCategory().canUse(context))
                    player.sendRawMessage(ChatColor.GRAY + "   [" + menu.getId() + ".] " + menu.getCategory().getChatName(context));
    ...
    static class ViewWarps extends ValidatingPrompt implements DemigodsConversation.Category {
        ...
        public boolean canUse(ConversationContext context) {
            return DemigodsPlayer.of((Player) context.getForWhom()).isACharacter();
        }
        ...
        public String getPromptText(ConversationContext context) {
            ...
            if (character.getMeta().hasWarps() || character.getMeta().hasInvites()) {
                ...
                for (Map.Entry<String, Object> entry : character.getMeta().getWarps().entrySet()) {
                    Location location = DemigodsLocation.get(UUID.fromString(entry.getValue().toString())).getBukkitLocation();
        ...
        protected boolean isInputValid(ConversationContext context, String message) {
            ...
            if ("menu".equalsIgnoreCase(message)) return true;
                ...
                if (StringUtils.isAlphanumeric(arg1) && !character.getMeta().getWarps().containsKey(arg1.toLowerCase()))
                    return true;
                notifications.add(English.NOTIFICATION_ERROR_CREATING_WARP);
            } else if ("warp".equalsIgnoreCase(arg0)) {
                if ((character.getMeta().getWarps().containsKey(arg1.toLowerCase()) || character.getMeta().getInvites().containsKey(arg1.toLowerCase())))
                    return true;
                ...
                if ((DemigodsCharacter.charExists(arg1) || (DemigodsPlayer.getFromName(arg1) != null && DemigodsPlayer.getFromName(arg1).isACharacter()) && arg2 != null && character.getMeta().getWarps().containsKey(arg2.toLowerCase())))
                    return true;
        ...
        protected Prompt acceptValidatedInput(ConversationContext context, String message) {
            ...
            if ("new".equalsIgnoreCase(arg0)) {
                ...
                // Define variables
                DemigodsCharacter invitee = DemigodsCharacter.charExists(arg1) ? Demigods.getServer().getCharacter(arg1) : DemigodsCharacter.of(Bukkit.getOfflinePlayer(arg1));
                Location warp = DemigodsLocation.get(UUID.fromString(character.getMeta().getWarps().get(arg2).toString())).getBukkitLocation(); // NPE (can't figure out why)

                // Add the invite
                invitee.getMeta().addInvite(character.getName(), warp);

                // Message the player if they're online
                if (invitee.getBukkitOfflinePlayer().isOnline()) {
                    invitee.getBukkitOfflinePlayer().getPlayer().sendMessage(ChatColor.LIGHT_PURPLE + "You've been invited to a warp by " + character.getName() + "!");
                    invitee.getBukkitOfflinePlayer().getPlayer().sendMessage(ChatColor.GRAY + "Go to an Altar to accept this invite.");
                }
                ...
                if (character.getMeta().getWarps().containsKey(arg1.toLowerCase()))
                    player.teleport(DemigodsLocation.get(UUID.fromString(character.getMeta().getWarps().get(arg1.toLowerCase()).toString())).getBukkitLocation());
                else if (character.getMeta().getInvites().containsKey(arg1.toLowerCase())) {
                    player.teleport(DemigodsLocation.get(UUID.fromString(character.getMeta().getInvites().get(arg1.toLowerCase()).toString())).getBukkitLocation());
        ...
        protected boolean isInputValid(ConversationContext context, String message) {
            ...
            try {
                ...
                if ("assign".equalsIgnoreCase(splitMsg[0]) && splitMsg.length >= 3) {
                    ...
                    Skill skill = DemigodsCharacter.of((Player) context.getForWhom()).getMeta().getSkill(skillType);
    ...
    static class ViewNotifications extends ValidatingPrompt implements DemigodsConversation.Category {
        ...
        protected Prompt acceptValidatedInput(ConversationContext context, String message) {
            ...
            if ("menu".equalsIgnoreCase(message)) {
                ...
                // Clear them
                for (String string : character.getMeta().getNotifications())
                    Notification.get(UUID.fromString(string)).remove();
    ...
    static class ViewCharacters extends ValidatingPrompt implements DemigodsConversation.Category {
        ...
        @Override
        public boolean canUse(ConversationContext context) {
            return DemigodsPlayer.of((Player) context.getForWhom()).getCharacters() != null && !DemigodsPlayer.of((Player) context.getForWhom()).getCharacters().isEmpty();
        }
        ...
        public String getPromptText(ConversationContext context) {
            ...
            for (DemigodsCharacter character : DemigodsPlayer.of(player).getCharacters())
        ...
        protected boolean isInputValid(ConversationContext context, String message) {
            String[] splitMsg = message.split(" ");
            DemigodsPlayer player = DemigodsPlayer.of((Player) context.getForWhom());
            DemigodsCharacter character = Demigods.getServer().getCharacter(splitMsg[0]);
            return "menu".equalsIgnoreCase(message) || splitMsg.length == 2 && (DemigodsPlayer.hasCharName((Player) context.getForWhom(), splitMsg[0]) && ("info".equalsIgnoreCase(splitMsg[1]) || (DemigodsPlayer.hasCharName((Player) context.getForWhom(), splitMsg[0]) && "switch".equalsIgnoreCase(splitMsg[1])) && (player.getCharacter() == null || !player.getCharacter().getName().equalsIgnoreCase(character.getName()))));
        }
        ...
        protected Prompt acceptValidatedInput(ConversationContext context, String message) {
            ...
            if ("info".equalsIgnoreCase(arg1)) {
                ...
                DemigodsPlayer.of((Player) context.getForWhom()).switchCharacter(Demigods.getServer().getCharacter(arg0));
            }
            return null;
        }
        ...
        class DetailedInfo extends ValidatingPrompt {
            ...
            public String getPromptText(ConversationContext context) {
                // Define variables
                Player player = (Player) context.getForWhom();
                DemigodsCharacter character = Demigods.getServer().getCharacter(context.getSessionData("viewing_character").toString());
            ...
            protected boolean isInputValid(ConversationContext context, String message) {
                ...
                return "back".equalsIgnoreCase(message) || ("switch".equalsIgnoreCase(message) && (player.getCharacter() == null || !player.getCharacter().getName().equalsIgnoreCase(character.getName())));
            }

            @Override
            protected Prompt acceptValidatedInput(ConversationContext context, String message) {
                if ("back".equalsIgnoreCase(message)) return new ViewCharacters();
                if ("switch".equalsIgnoreCase(message))
                    DemigodsPlayer.of((Player) context.getForWhom()).switchCharacter(Demigods.getServer().getCharacter(context.getSessionData("viewing_character").toString()));
                return null;
            }
        }
    }
    ...
    static class CreateCharacter extends ValidatingPrompt implements DemigodsConversation.Category {
        ...
        class ChooseName extends ValidatingPrompt {
            ...
            protected boolean isInputValid(ConversationContext context, String name) {
                if (name.length() < 2 || name.length() > 13 || !StringUtils.isAlphanumeric(name) || Strings.hasCapitalLetters(name, Configs.getSettingInt("character.max_caps_in_name")) || DemigodsCharacter.charExists(name) || Strings.containsAnyInCollection(name, getBlackList())) {
                    // Create the list
                    ...
                    if (Strings.hasCapitalLetters(name, Configs.getSettingInt("character.max_caps_in_name")))
                        errors.add(English.ERROR_MAX_CAPS);
                    if (DemigodsCharacter.charExists(name) || Strings.containsAnyInCollection(name, getBlackList()))
                        errors.add(English.ERROR_CHAR_EXISTS);
        ...
        class ChooseAlliance extends ValidatingPrompt {
            ...
            public String getPromptText(ConversationContext context) {
                ...
                for (Alliance alliance : Demigods.getMythos().getAlliances())
                    if (player.hasPermission(alliance.getPermission()) && alliance.isPlayable() && Alliance.Util.getLoadedMajorPlayableDeitiesInAllianceWithPerms(alliance, player).size() > 0)
                        player.sendRawMessage(ChatColor.GRAY + "    " + Symbol.RIGHTWARD_ARROW + " " + ChatColor.YELLOW + StringUtils.capitalize(alliance.getName().toLowerCase()));
        ...
        class ChooseDeity extends ValidatingPrompt {
            ...
            public String getPromptText(ConversationContext context) {
                ...
                for (Deity deity : Alliance.Util.getLoadedMajorPlayableDeitiesInAllianceWithPerms((Alliance) context.getSessionData("chosen_alliance"), player))
                    if (player.hasPermission(deity.getPermission()))
                        player.sendRawMessage(ChatColor.GRAY + "    " + Symbol.RIGHTWARD_ARROW + " " + (deity.getFlags().contains(Deity.Flag.DIFFICULT) ? ChatColor.DARK_RED : ChatColor.YELLOW) + StringUtils.capitalize(deity.getName()) + ChatColor.GRAY + " - " + deity.getShortDescription());
            ...
            protected boolean isInputValid(ConversationContext context, String deityName) {
                return Demigods.getMythos().getDeity(deityName) != null && Demigods.getMythos().getDeity(deityName).getFlags().contains(Deity.Flag.PLAYABLE) && ((Player) context.getForWhom()).hasPermission(Demigods.getMythos().getDeity(deityName).getPermission());
            }
        ...
        class ConfirmDeity extends ValidatingPrompt {
            ...
            public String getPromptText(ConversationContext context) {
                Messages.clearRawChat((Player) context.getForWhom());
                Deity deity = Demigods.getMythos().getDeity((String) context.getSessionData("chosen_deity"));
            ...
            protected Prompt acceptValidatedInput(ConversationContext context, String message) {
                if (StringUtils.contains(message, "y")) {
                    ...
                    player.sendRawMessage(" ");
                    for (Map.Entry<Material, Integer> entry : Demigods.getMythos().getDeity(chosenDeity).getClaimItems().entrySet())
    ...
    // DemigodsCharacter confirmation
    ...
    // DemigodsCharacter creation cancellation
    ...
    public static class Listener implements org.bukkit.event.Listener {
        ...
        public void prayerInteract(PlayerInteractEvent event) {
            ...
            if (DemigodsStructureType.Util.isClickableBlockWithFlag(event.getClickedBlock().getLocation(), DemigodsStructureType.Flag.PRAYER_LOCATION)) {
                if (!Util.isPraying(player)) {
                    if (DemigodsPlayer.of(player).canPvp()) {
                    ...
                    for (Entity entity : player.getNearbyEntities(20, 20, 20))
                        if (entity instanceof Player)
                            ((Player) entity).sendMessage(ChatColor.AQUA + English.KNELT_FOR_PRAYER.getLine().replace("{player}", ChatColor.stripColor(player.getDisplayName())));
        ...
        public void confirmDeity(final InventoryCloseEvent event) {
            try {
                ...
                final Deity deity = Demigods.getMythos().getDeity((String) prayerContext.getSessionData("chosen_deity"));
        ...
        private void onPlayerMove(PlayerMoveEvent event) {
            ...
            if (Util.isPraying(player) && event.getTo().distance((Location) TempDataManager.getValueTemp(player.getName(), "prayer_location")) >= Configs.getSettingInt("zones.prayer_radius"))
                Util.togglePraying(player, false);
        }
    }
    ...
    public static class Util {
        ...
        public static ConversationContext getPrayerContext(Player player) {
            ...
            return (ConversationContext) TempDataManager.getValueTemp(player.getName(), "prayer_context");
        }
        ...
        public static void togglePrayingSilent(Player player, boolean option, boolean recordChat) {
            if (option) {
                ...
                TempDataManager.saveTemp(player.getName(), "prayer_conversation", conversation);
                TempDataManager.saveTemp(player.getName(), "prayer_location", player.getLocation());
                player.setSneaking(true);

                // Record chat if enabled
                if (recordChat) DemigodsPlayer.of(player).startRecording();
            } else {
                // Save context and abandon the conversation
                if (TempDataManager.hasKeyTemp(player.getName(), "prayer_conversation")) {
                    Conversation conversation = (Conversation) TempDataManager.getValueTemp(player.getName(), "prayer_conversation");
                    TempDataManager.saveTemp(player.getName(), "prayer_context", conversation.getContext());
                ...
                DemigodsPlayer.of(player).stopRecording(recordChat);
            }
        }
    }
}
