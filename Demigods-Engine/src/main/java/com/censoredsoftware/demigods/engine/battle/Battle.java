...
public class Battle extends DataAccess<UUID, Battle>
{
	...
	{
		return DemigodsCharacter.Util.load(startedBy);
	}
	...
	public Set<Participant> getParticipants()
	{
		return Sets.filter(Sets.union(Sets.newHashSet(Collections2.transform(involvedPlayers, new Function<String, Participant>()
		{
			...
			{
				return DemigodsCharacter.Util.load(UUID.fromString(character));
			}
	...
	public Map<UUID, Integer> getScores()
	{
		...
		{
			if(!getParticipants().contains(DemigodsCharacter.Util.load(UUID.fromString(entry.getKey())))) continue;
	...
	public int getScore(final Alliance alliance)
	{
		...
		for(int i : Collections2.transform(Collections2.filter(score.entrySet(), new Predicate<Map.Entry<UUID, Integer>>()
		{
			...
			{
				return DemigodsCharacter.Util.load(entry.getKey()).getAlliance().getName().equalsIgnoreCase(alliance.getName());
			}
	...
	public Collection<DemigodsCharacter> getMVPs()
	{
		...
		}), new Function<Map.Entry<UUID, Integer>, DemigodsCharacter>()
		{
			...
			{
				return DemigodsCharacter.Util.load(entry.getKey());
			}
		});
	}
	...
	public void end() // TODO Make this specify that it was a pet that won/lost a duel
	{
		for(String stringId : involvedPlayers)
			DemigodsServer.TIMED.setBool(stringId + "just_finished_battle", true, 1, TimeUnit.MINUTES);
		...
		if(participants.size() == 2)
		{
			...
			{
				DemigodsCharacter one = DemigodsCharacter.Util.load(participants.get(0));
				DemigodsCharacter two = DemigodsCharacter.Util.load(participants.get(1));
			...
			else
			{
				...
				DemigodsCharacter winner = DemigodsCharacter.Util.load(participants.get(winnerIndex));
				DemigodsCharacter loser = DemigodsCharacter.Util.load(participants.get(winnerIndex == 0 ? 1 : 0));
	...
	public void sendMessage(String message)
	{
		for(String stringId : involvedPlayers)
		{
			OfflinePlayer offlinePlayer = DemigodsCharacter.Util.load(UUID.fromString(stringId)).getOfflinePlayer();
	...
	public static Participant defineParticipant(Entity entity)
	{
		...
		if(entity instanceof Player) return DemigodsPlayer.Util.getPlayer((Player) entity).getCurrent();
