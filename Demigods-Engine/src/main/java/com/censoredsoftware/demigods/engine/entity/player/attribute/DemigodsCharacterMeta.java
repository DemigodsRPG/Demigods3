...
public class DemigodsCharacterMeta extends DataAccess<UUID, DemigodsCharacterMeta>
{
	...
	void setCharacter(DemigodsCharacter character)
	{
	...
	{
		return DemigodsCharacter.Util.load(character);
	}
	...
	{
		if(getRawSkills().containsKey(type.toString())) return Skill.Util.get(UUID.fromString(getRawSkills().get(type.toString()).toString()));
		return null;
	}
	...
	public Collection<Skill> getSkills()
	{
		return Collections2.transform(getRawSkills().values(), new Function<Object, Skill>()
		{
			...
			{
				return Skill.Util.get(UUID.fromString(obj.toString()));
			}
		});
	}
