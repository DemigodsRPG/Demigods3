		...
		plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable()
		{
			...
			public void run()
			{
				for(Player player : Bukkit.getOnlinePlayers())
				{
					...
					{				
						if(Long.valueOf(DSave.getPlayerData(player.getName(), "was_PVP_temp").toString()) + (double) (DConfig.getSettingDouble("pvp_area_delay_seconds") * 20) > System.currentTimeMillis()) continue;
