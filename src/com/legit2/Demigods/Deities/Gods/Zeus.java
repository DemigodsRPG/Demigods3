...
public class Zeus implements Listener
{
	...
	{
		if(DUtil.hasDeity(player.getName(), getName()) && DUtil.isImmortal(player.getName()))
		{
	...
	public static void lightningCommand(Player player, String arg1)
	{
		...
		if(arg1.equalsIgnoreCase("bind"))
		{
			...
			{
				if(DUtil.isBound(player.getName(), player.getItemInHand().getType())) player.sendMessage(ChatColor.YELLOW+"That item is already bound to a skill.");
				if(player.getItemInHand().getType() == Material.AIR) player.sendMessage(ChatColor.YELLOW+"You cannot bind a skill to air.");
				else
				{
					DUtil.setBound(player.getName(), player.getItemInHand().getType());
			...
			{
				DUtil.removeBind(player.getName(), LIGHTNINGBIND);
	...
	public static void shoveCommand(Player player, String[] args)
	{
		...
		if(args.length == 1 && args[0].equalsIgnoreCase("bind"))
		{
			...
			{
				DUtil.removeBind(player.getName(), SHOVEBIND);
