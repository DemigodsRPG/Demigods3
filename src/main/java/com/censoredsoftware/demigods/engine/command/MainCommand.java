...
public class MainCommand extends WrappedCommand {
                                ...
                                for (String toPrint : new ArrayList<String>() {
                                    {
                                        ...
                                        for (Ability ability : deity.getAbilities()) {
                                            for (String detail : ability.getDetails()) {
                                                ...
                                                if (ability.getCommand() != null)
                                                    details.append(ChatColor.GREEN + "/").append(ability.getCommand().toLowerCase()).append(ChatColor.WHITE).append(": ");
                                                details.append(ChatColor.WHITE).append(detail);
                                                add(details.toString());
                                                if (ability.getCost() > 0)
                                                    add("      " + ChatColor.GRAY + "-" + ChatColor.WHITE + " Favor cost: " + ChatColor.RED + ability.getCost());
                                            }
                                        }
                                        add(" ");
                                    }
    ...
    private static boolean dg_admin(Player sender, String[] args) {
        ...
        for (AdminCommands command : AdminCommands.values()) {
            if (args[1].equalsIgnoreCase(command.getCommand().getRootCommand()))
                return command.getCommand().doCommand(sender, args);
        }
            ...
            if (args[2].equalsIgnoreCase("player")) {
                ...
                if (player != null) {
                    ...
                    if (player.hasCurrent())
                        sender.sendMessage(ChatColor.GRAY + " " + Symbol.RIGHTWARD_ARROW + " " + ChatColor.RESET + "Current Character: " + player.getCurrent().getDeity().getColor() + player.getCurrent().getName());
