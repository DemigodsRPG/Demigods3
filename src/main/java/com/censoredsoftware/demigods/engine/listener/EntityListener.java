...
public class EntityListener implements Listener {
    ...
    public static void damageEvent(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof Player && !Battle.Util.canTarget(Battle.Util.defineParticipant(event.getEntity())))
    ...
    public static void damageByEntityEvent(EntityDamageByEntityEvent event) {
        ...
        if (attacker instanceof Player) {
            ...
            if (attacked instanceof Tameable && ((Tameable) attacked).isTamed() && DPet.Util.getPet((LivingEntity) attacked) != null && DPlayer.Util.getPlayer(hitting).getCurrent() != null && DCharacter.Util.areAllied(DPlayer.Util.getPlayer(hitting).getCurrent(), DPet.Util.getPet((LivingEntity) attacked).getOwner()))
                event.setCancelled(true);
        }
    }
    ...
    public void onTame(EntityTameEvent event) {
        ...
        if (character != null)
            DPet.Util.create((Tameable) entity, DPlayer.Util.getPlayer(Bukkit.getOfflinePlayer(owner.getName())).getCurrent());
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onEntityTarget(EntityTargetLivingEntityEvent event) {
        if (Zones.inNoDemigodsZone(event.getEntity().getLocation())) return;
        if (event.getTarget() instanceof Player && !DPlayer.Util.getPlayer(((Player) event.getTarget())).canPvp())
            event.setCancelled(true);
    }
}
