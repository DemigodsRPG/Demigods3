...
public class Prayer implements WrappedConversation {
    ...
    public static Conversation startPrayer(Player player) {
        try {
            ...
            if (!Demigods.Util.isRunningSpigot()) {
                ...
                if (DataManager.hasKeyTemp(player.getName(), "prayer_context"))
                    conversationContext = (Map<Object, Object>) sessionDataField.get(DataManager.getValueTemp(player.getName(), "prayer_context"));
            } else {
                // Grab the context Map
                if (DataManager.hasKeyTemp(player.getName(), "prayer_context"))
                    conversationContext.putAll(((ConversationContext) DataManager.getValueTemp(player.getName(), "prayer_context")).getAllSessionData());
            }
    ...
    static class ViewWarps extends ValidatingPrompt implements Demigods.DemigodsConversation.Category {
        ...
        protected Prompt acceptValidatedInput(ConversationContext context, String message) {
            ...
            if (arg0.equalsIgnoreCase("new")) {
                ...
                Location warp = DLocation.Util.load(UUID.fromString(character.getMeta().getWarps().get(arg2).toString())).toLocation(); // NPE (can't figure out why)
                ...
                if (character.getMeta().getWarps().containsKey(arg1.toLowerCase()))
                    player.teleport(DLocation.Util.load(UUID.fromString(character.getMeta().getWarps().get(arg1.toLowerCase()).toString())).toLocation());
                else if (character.getMeta().getInvites().containsKey(arg1.toLowerCase())) {
        ...
        class ChooseAlliance extends ValidatingPrompt {
            ...
            public String getPromptText(ConversationContext context) {
                ...
                for (Alliance alliance : Demigods.MYTHOS.getAlliances())
                    if (player.hasPermission(alliance.getPermission()) && alliance.isPlayable())
    ...
    public static class Listener implements org.bukkit.event.Listener {
                ...
                if (!DPlayer.Util.isPraying(player)) {
                    ...
                    for (Entity entity : player.getNearbyEntities(20, 20, 20))
                        if (entity instanceof Player)
                            ((Player) entity).sendMessage(ChatColor.AQUA + Demigods.LANGUAGE.getText(Translation.Text.KNELT_FOR_PRAYER).replace("{player}", ChatColor.stripColor(player.getDisplayName())));
        ...
        private void onPlayerMove(PlayerMoveEvent event) {
            ...
            if (DPlayer.Util.isPraying(player) && event.getTo().distance((Location) DataManager.getValueTemp(player.getName(), "prayer_location")) >= Configs.getSettingInt("zones.prayer_radius"))
                DPlayer.Util.togglePraying(player, false);
        }
    }
}
